...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  if (!firebaseEnabled) {
    ...
    const url = `${siteUrl}/experiencia/${slug}`;
    return {
  ...
  return {
    ...
    keywords: [paquete.titulo, paquete.destino || 'destino', 'experiencias', 'viajes', 'turismo', SITE_NAME],
  };
}
...
export default async function ExperienciaPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <div className="min-h-screen bg-white">
      <PaqueteSchema paquete={paquete} basePath="/experiencia" />
      <Navbar variant="homeMockup" reserveSpace />
      <WhatsAppButton />
      ...
      <main className="container mx-auto px-4 py-6 md:px-6 md:py-8 lg:px-8">
        <div className="text-xs text-gray-500 md:text-sm">
          ...
          <Link href="/experiencias" className="hover:text-black">
            Excursiones
          </Link>
          <span className="mx-2">›</span>
              ...
              <div>
                ...
                {longDescriptionHtml ? (
                  ...
                  <p className="mt-4 text-sm leading-relaxed text-gray-700">
                    Descubrí paisajes inolvidables y experiencias únicas con un programa premium que combina actividades, alojamientos seleccionados y servicios exclusivos.
                  </p>
                )}
              </div>
