...
export default function Navbar({ transparent = false, forceTransparent = false, reserveSpace = false, theme = "default" }: NavbarProps) {
  ...
  const handleSearch = (e: React.FormEvent) => {
    e.preventDefault();
    if (searchQuery.trim()) {
      router.push(`/experiencias?q=${encodeURIComponent(searchQuery.trim())}`);
      setSearchQuery("");
    }
  };
        ...
        <div className={`mx-auto transition-all duration-500 ${
          ...
          <div className="hidden lg:flex items-center justify-center flex-1 mx-4 text-[11px] xl:text-[12px] font-semibold text-gray-500 tracking-[0.08em]">
            <div className="flex items-center space-x-5">
              ...
              <Link href="/experiencias" className="hover:text-primary transition-colors uppercase">Experiencias</Link>
            ...
            >
              <div className="relative h-full flex flex-col p-6">
                ...
                <nav className="space-y-1 text-left flex-1">
                  ...
                  <Link href="/experiencias" className="flex items-center gap-3 rounded-xl px-3 py-3 text-sm font-semibold text-gray-800 hover:bg-gray-50 hover:text-primary transition" onClick={() => setMobileMenuOpen(false)}>
                    Experiencias
                  </Link>
